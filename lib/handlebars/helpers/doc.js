...
function getRelatedPartials(assemble, options) {
    ...
    while ((m = re.exec(options.fn())) !== null) {
        ...
        if (RegExp(/^node_modules\//).test(relatedPartialURI)) {
            // package
            relativePath = upath.relative(partialURI.replace(/(test\/tmpl\/partials\/)/, 'node_modules/'), relatedPartialURI.replace(upath.extname(relatedPartialURI), '.html'));
            if (RegExp(/\.\.\/[^\/]*-pkg-/).test(relativePath)) {
                relativePath = relativePath.replace(/\.\.\//, '../../packages/');
            }
        } else {
            if (RegExp(/^src\/[^tmpl\/]/).test(relatedPartialURI)) {
                // local package
                relativePath = upath.relative(partialURI.replace(/(test\/tmpl\/partials\/)/, 'src/'), relatedPartialURI.replace(upath.extname(relatedPartialURI), '.html'));
            } else {
                // local
                relativePath = upath.relative(upath.dirname(partialURI.replace(/(src|test)\/tmpl\//, '')), relatedPartialURI.replace(upath.extname(relatedPartialURI), '.html')).replace(/(src|test)\/tmpl\//, '');
            }
        }
        list.push({
            title: partial.key,
