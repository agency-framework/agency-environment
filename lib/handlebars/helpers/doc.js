...
module.exports = function(data) {
    ...
    return function(options, cb) {
        ...
        data.assemble.engine('.hbs').render(options.fn(), options.data.root, function(err, html) {
            if (err) {
                return cb(err);
            }
            var tmpl = template({
                ...
                code: {
                    ...
                    json: hljs.highlightAuto(getJSONContent(data.assemble, options), ['json']).value
                },
                info: getRelatedPartials(data.assemble, options)
            });
            cb(null, tmpl);
        });
    };
...
function getJSONContent(assemble, options) {
    var partialName = upath.join(options.data.root.partial).replace('partials/', '');
    var partial = assemble.views.partials[partialName];

    if (partial) {
        var data = assemble.views.partials[partialName].context();
