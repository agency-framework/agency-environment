...
function getRelatedPartials(assemble, options) {
    ...
    while ((m = re.exec(options.fn())) !== null) {
        ...
        if (RegExp(/^node_modules\//).test(relatedPartialURI)) {
            ...
            relativePath = upath.join(upath.relative('/' + options.data.root.partial, '/'), 'packages', relativePath);
            relativePath = upath.join(options.data.root.relativeToRoot.replace(/^\.\.\/\.\.\/\.\.\//, ''), relativePath);
        } else {
