...
function getRelatedPartials(assemble, options) {
    ...
    while ((m = re.exec(options.fn())) !== null) {
        ...
        if (relatedPartialURI.match(/^node_modules\//)) {
            // package
            relativePath = upath.relative(partialURI.replace(/test\/tmpl\/partials\//, ''), relatedPartialURI.replace(/^node_modules\//, 'packages/').replace(upath.extname(relatedPartialURI), '.html'));
        } else {
            // local
