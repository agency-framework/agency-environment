...
module.exports = function(assemble) {
    return function(options, cb) {
        options.data.root.partial = renameKey(options.data.root.partial);
...
function getRelatedPartials(assemble, options) {
    ...
    while ((m = re.exec(options.fn())) !== null) {
        ...
        if (relatedPartialURI.match(/^node_modules\//)) {
            ...
            // local
            relativePath = upath.relative(upath.dirname(partialURI), 'docs/'+relatedPartialURI.replace(upath.extname(relatedPartialURI), '.html')).replace(/src\/tmpl\//, '');
        }
