...
var defaultDestJS = upath.normalize('src/js/packages.js'),
    defaultDestPCSS = upath.normalize('src/js/packages.js');
...
module.exports = {
    ...
    prepare: function(config, cb) {
        config = config || {};
        writeFile(config.js || defaultDestJS, '', function(){
            writeFile(config.pcss || defaultDestPCSS, '', cb);
        });
    },
    ...
    createControllerRegistry: function(config, registry, cb) {
        ...
        writeFile((registry || {}).file || config.js || defaultDestJS, controllerTemplate({
    ...
    createPartialRegistry: function(config, registry, cb) {
        ...
        var dest = upath.normalize(config.pcss || defaultDestPCSS);
        // partial
...
function addPkgToList(nodes) {
    ...
    for (var i = 0; i < nodes.length; i++) {
        ...
        partialList.push({
            name: partial,
            path: path,
            filePath: upath.normalizeSafe('./' + upath.join('node_modules', filePath.join('/')))
        });
    }
}
