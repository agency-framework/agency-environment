    ...
    return through.obj(function(file, enc, cb) {
        if (file.contents) {
            ...
            scope.fileDataList.push({
                ...
                controllers: getControllersList(scope, $('.controller[data-controller]')),
                partials: getPartialsList(scope, $('.partial[data-partial]')).concat(getPkgList(scope, $('.pkg[data-pkg]')))
            });
        }
        cb();
    });
...
function createRegistryPartials(list, scope, config, cb) {
    ...
    filter(list, function(entry) {
        ...
        if (fs.existsSync(entry.filePath)) {
            ...
            console.log('[' + 'task'.gray + '][' + 'handlebars'.gray + ']', 'added'.bold.green, entry.name.gray, 'import to', upath.basename(entry.critical ? criticalPath : path).gray);
        } else {
    ...
    writeFile(path, partialTemplate({
        ...
        console.log('saved file:', upath.basename(path));
        writeFile(criticalPath, partialTemplate({
            sources: sources.critical
        }), function() {
            console.log('saved file:', upath.basename(criticalPath));
            cb();
        });
    });
}
...
function getPartialsList(scope, nodes) {
...
function getPkgList(scope, nodes) {
