...
assemble.asyncHelper('concat', require('../handlebars/helpers/concat'));
...
// console.log(view.options);
    if(upath.extname(view.locals.relative) === '.hbs') {

        // console.log(view.locals);
        var key = view.locals.relative.replace(upath.extname(view.locals.relative), '');
        if(view.locals.base.indexOf('partials/') > -1) {
            key = 'partials/' + view.locals.relative.replace(upath.extname(view.locals.relative), '');
        }

        if(this.cache.data[key]) {
            lazy.mixin(context, this.cache.data[key]);
        }
        // console.log('OHO', view.data);
        // console.log('------------------------');
    }
