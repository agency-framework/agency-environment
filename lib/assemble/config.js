...
assemble.helper('raw', require('./helpers/raw'));
assemble.helper('doc', require('./helpers/doc'));
assemble.helper('test', require('./helpers/test'));

assemble.option('renameKey', function(filepath, options) {
    return path.relative(process.cwd() + '/src/tmpl/', filepath).replace('partials/', '').replace(path.extname(filepath), '');
});
