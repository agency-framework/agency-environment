...
function getRelativeToRoot(view) {
    ...
    } else if (view.options.plural === 'docs-packages' || view.options.plural === 'src-docs' || view.options.plural === 'partial-docs' || view.options.plural === 'docs-partials') {
        relativeToRoot = upath.relative('\/docs\/' + upath.join(upath.dirname(view.key.replace(process.cwd(), '').replace(/(src|test)\/tmpl\//, ''))), '/'); //'\/..\/..';
    }
    if (relativeToRoot !== '') {
