                            ...
                            resolve: function(id, options) {
                                ...
                                try {
                                    if (task.packagePattern.find(function(value) {
                                        ...
                                        console.log('BAAAm'.magenta.bold, id);
                                        if (RegExp(/\.pcss$/).test(id)) {
                                            ...
                                            if (fs.statSync(path).isDirectory()) {
                                                ...
                                                path = upath.join(path, 'index.pcss');
                                            } else {
                                                path = path;
                                            }
                                        } else {
                                            path = upath.resolve(process.cwd(), 'node_modules', id, 'index.pcss');
                                        }
                                    } else {
                                    ...
                                    console.log('[' + 'POSTCSS TASK'.gray + ']', 'can\'t find file'.bold.red, path.bold.red);
                                }
                                return path;
                            }
                        });
