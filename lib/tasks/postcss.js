                    ...
                    if (plugin.name === 'postcss-import') {
                        plugin.params.push({
                            resolve: function(id, options) {
                                var newPath = options.baseDir + id;
                                if (task.packagePattern.find(function(value) {
                                    ...
                                    if (RegExp(/\.pcss$/).test(id)) {
                                        ...
                                        newPath = 'node_modules/' + id.replace(/\.\//, '') + '/src/pcss/index.pcss';
                                    }
                                    return process.cwd() + '/' + newPath;
                                } else {
                                    return process.cwd() + '/src/pcss' + id.replace(/\.css$/, '.pcss').replace(/^\./, '');
                                }
                            }
                        });
                    }
