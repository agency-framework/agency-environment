...
function createGridOffset(dest, breakpoints, columns, prefix, cb) {
    if (breakpoints) {
        ...
        for (key in breakpoints) {
            if (breakpoints.hasOwnProperty(key)) {
                css += '@media screen and (min-width: ' + breakpoints[key] + ')' + (nextBreakpoint[key] ? ' and (max-width: ' + nextBreakpoint[key] + ')' : '') + ' {\n';
                for (var i = 1; i <= columns; i++) {
                    css += '   .' + prefix + '-' + key + '-offset-' + (i) + '-' + columns + ' {\n';
                    css += '       margin-left: ' + ((i / columns) * 100) + '\%;\n';
                    css += '   } \n';
                }
                css += '}\n';
            }
