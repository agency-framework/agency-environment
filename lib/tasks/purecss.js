...
module.exports = function(config) {
    return function (cb) {
        createGrid(config.files.dest + 'custom-grid.css', config.breakpoints, function() {
            createCustomMediaVariables(config.files.dest + 'custom-media-variables.pcss', config.columns, config.breakpoints, function() {
                cb();
            });
        });
    };
...
function createGrid(dest, columns, map, cb) {
    var css = rework('').use(pureGrids.units(columns || 12, {
        decimals: 4,
